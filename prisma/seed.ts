...
async function main() {
  ...
  await prisma.quiz.create({
    data: {
      ...
      questions: { create: sampleQuiz.map((q) => ({ promptAz: q.question, options: q.options, answer: q.answer, explanationAz: "Doğru yanaşma: verilənləri oxu, uyğun əməliyyatı seç və nəticəni yoxla.", lesson: { connect: { id: lesson.id } } })) }
    }
  });
}
